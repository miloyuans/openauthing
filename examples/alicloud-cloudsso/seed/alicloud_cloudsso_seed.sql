...
INSERT INTO applications (
    ...
    '',
    'Alibaba Cloud CloudSSO SAML demo service provider'
)
...
INSERT INTO saml_service_providers (
    ...
    '',
    '{"username":"username","email":"email","name":"display_name","groups":"groups"}'::jsonb
)
...
INSERT INTO applications (
    ...
    '',
    'Alibaba Cloud CloudSSO SCIM target placeholder; keep the SCIM bearer token outside the database'
)
ON CONFLICT (tenant_id, code) DO UPDATE
SET name = EXCLUDED.name,
    type = EXCLUDED.type,
...
INSERT INTO groups (
    ...
    :'group_name',
    :'group_code',
    'Alibaba Cloud CloudSSO demo group for SCIM sync'
)
ON CONFLICT (tenant_id, code) DO UPDATE
SET name = EXCLUDED.name,
    description = EXCLUDED.description,
